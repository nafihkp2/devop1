...
const HRDashboard = () => {
  ...
  const fetchTeamsAndProjects = async () => {
    try {
      const headers = { userid: userId, adminid: userId };
      const teamsResponse = await axios.get(`${host}/api/dash/teams`, { headers });
      setTeams(teamsResponse.data);

      const initialProjects = teamsResponse.data.reduce((acc, team) => ({
        ...acc,
        [team.id]: {
          task: '',
          startDateTime: '',
      ...
      const projectsResponses = await Promise.all(
        teamsResponse.data.map(team => axios.get(`${host}/api/dash/projects/${team.id}`, { headers }))
      );
